...
export class UserTable extends BaseEntity 
{
    ...
    @ManyToOne(type => UserTypeTable, user_type_table=>user_type_table.user_tables, {eager : false})
    ...
    @OneToMany(type=>CommentTable, comment_table => comment_table.user, {eager : true})
    comment_tables : CommentTable[];

    @OneToMany(type=>BoardTable, board_table => board_table.user, {eager : true})
    board_tables : BoardTable[];

    @OneToMany(type=>Attendance, attendance=>attendance.user, {eager : true})
