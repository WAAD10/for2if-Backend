...
export class BoardTable extends BaseEntity
{
    ...
    @ManyToOne(type=>BoardCategory, board_category => board_category.board_tables, {eager : false})
    board_category : BoardCategory;

    @Column({type : 'character varying'})
    board_content : string;

    @ManyToOne(type=>UserTable, user_table => user_table.board_tables, {eager : false})
    user : UserTable;

    ///////////////////////////////////////////////////////////////////////////////////////////////

    @OneToMany(type=>BoardImage, board_image => board_image.board, {eager : true})
    board_images : BoardImage[];

    @OneToMany(type=>CommentTable, comment_table => comment_table.board, {eager : true})
